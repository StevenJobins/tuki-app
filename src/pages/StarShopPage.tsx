    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="pb-24 overflow-x-hidden">
      ...
      <div className="mx-4 mt-4 bg-gradient-to-br from-yellow-400 via-amber-400 to-orange-400 rounded-2xl p-6 text-white shadow-lg">
        <div className="text-center">
          ...
          <div className="text-white/90 text-sm">{child ? `${child.name}s Sterne-Guthaben` : 'Dein Sterne-Guthaben'}</div>
          <div className="mt-3 flex items-center justify-center gap-4 text-xs text-white/80">
            <span>{tukiStars.total} verdient</span><span>{'·'}</span><span>{tukiStars.spent} ausgegeben</span>
          </div>
          <div className="mt-2 bg-white/20 rounded-full px-3 py-1 inline-block text-xs">{tukiStars.levelName}</div>
        </div>
      </div>
      ...
      <div className="px-4 grid grid-cols-2 gap-3">
        {filtered.map(reward => {
          ...
          return (
            <motion.div key={reward.id} layout initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              className={`bg-white rounded-xl p-4 shadow-sm border transition-all ${isRedeemed ? 'border-green-200 bg-green-50/50' : canAfford ? 'border-gray-100 hover:border-amber-300 hover:shadow-md cursor-pointer' : 'border-gray-100 opacity-60'}`}
            ...
            >
              ...
              <div className="mt-3 flex items-center justify-between">
                {isRedeemed ? (
                  <span className="text-xs font-medium text-green-600 flex items-center gap-1">{wasJustRedeemed ? '🎉' : '✅'} Eingelöst</span>
                ) : (
                  <>
                    <span className={`text-sm font-bold ${canAfford ? 'text-amber-600' : 'text-gray-400'}`}>{reward.price} {'⭐'}</span>
                    {canAfford && <span className="text-xs text-amber-600 font-medium">Einlösen</span>}
                  </>
                )}
              </div>
            </motion.div>
      ...
      {redeemedRewards.length > 0 && (
        <div className="mx-4 mt-6 mb-4">
          <h3 className="text-sm font-semibold text-tuki-brown mb-2">{'🌟'} Eingelöste Belohnungen ({redeemedRewards.length})</h3>
          <div className="flex flex-wrap gap-2">
            {redeemedRewards.map((rId: string) => {
              const r = REWARDS.find(rew => rew.id === rId)
              if (!r) return null
              return (
                <div key={rId} className="bg-green-50 border border-green-200 rounded-full px-3 py-1 text-xs text-green-700 flex items-center gap-1">{r.emoji} {r.name}</div>
              )
            })}
          </div>
        </div>
            ...
            >
              <div className="text-center">
                ...
                <div className="mt-4 bg-amber-50 rounded-xl p-3">
                  <span className="text-amber-700 font-medium">{selectedReward.price} {'⭐'} einlösen?</span>
                  <div className="text-xs text-amber-600 mt-1">Verbleibendes Guthaben: {balance - selectedReward.price} Sterne</div>
                </div>
                <div className="flex gap-3 mt-5">
                  <button onClick={() => setSelectedReward(null)} className="flex-1 py-3 rounded-xl border border-gray-200 text-gray-600 font-medium">Abbrechen</button>
                  <button onClick={confirmRedeem} className="flex-1 py-3 rounded-xl bg-gradient-to-r from-amber-500 to-orange-500 text-white font-medium shadow-md">Einlösen {'⭐'}</button>
                </div>
              </div>
            </motion.div>
          </motion.div>
