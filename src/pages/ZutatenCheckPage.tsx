  ...
  recipes.forEach(r => {
    r.ingredients.forEach(i => {
      ...
      if (name.includes(':')) {
        ...
        parts.forEach(p => { const normalized = normalizeIngredient(p); if (normalized.length > 1) all.add(normalized) })
      } else if (name.includes(',')) {
        const parts = name.split(',')
        parts.forEach(p => { const normalized = normalizeIngredient(p); if (normalized.length > 1) all.add(normalized) })
      } else {
        const normalized = normalizeIngredient(i.item)
...
export default function ZutatenCheckPage() {
  ...
  const suggestions = searchInput.length >= 2
    ? allIngredients.filter(i => i.includes(searchInput.toLowerCase()) && !selectedIngredients.includes(i)).slice(0, 6)
    : []
  ...
  const addQuickItem = (item: string) => {
    if (!selectedIngredients.includes(item)) setSelectedIngredients(prev => [...prev, item])
  }

  const recipeMatches = useMemo(() => {
    if (selectedIngredients.length === 0) return []
    return recipes.map(recipe => {
      const recipeIngredients = recipe.ingredients.map(i => normalizeIngredient(i.item))
      const matched = recipeIngredients.filter(ri => selectedIngredients.some(si => ri.includes(si) || si.includes(ri)))
    ...
    }).filter(m => m.matched > 0).sort((a, b) => b.percentage - a.percentage || a.missing - b.missing)
  }, [selectedIngredients])
  ...
  return (
    <div className="pb-24">
      ...
      <div className="px-4 mb-4">
        <p className="text-sm text-gray-500">Wähle die Zutaten aus, die du zuhause hast — wir zeigen dir passende Rezepte!</p>
      </div>
      <div className="px-4 mb-3">
        <div className="relative">
          ...
          <input type="text" placeholder="Zutat eingeben..." value={searchInput} onChange={e => setSearchInput(e.target.value)}
        ...
        {suggestions.length > 0 && (
          <div className="mt-1 bg-white rounded-xl border border-gray-200 shadow-lg overflow-hidden">
            {suggestions.map(s => (
              <button key={s} onClick={() => toggleIngredient(s)}
                className="w-full px-4 py-2.5 text-left text-sm text-gray-700 hover:bg-tuki-mint/20 transition-colors border-b border-gray-50 last:border-0 capitalize">+ {s}</button>
            ))}
          </div>
        )}
      </div>
      {selectedIngredients.length > 0 && (
        <div className="px-4 mb-4">
          <div className="flex flex-wrap gap-2">
            ...
            <button onClick={() => setSelectedIngredients([])} className="px-3 py-1.5 text-xs text-gray-400 hover:text-gray-600">Alle löschen</button>
          </div>
        </div>
      )}
      {selectedIngredients.length === 0 && (
            ...
            <div key={cat.label} className="mb-3">
              ...
              <div className="flex flex-wrap gap-1.5">
                {cat.items.map(item => {
                  ...
                  return (
                    <button key={item} onClick={() => isSelected ? toggleIngredient(item) : addQuickItem(item)}
                      className={`px-2.5 py-1 rounded-full text-xs transition-colors capitalize ${isSelected ? 'bg-tuki-rot text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                      {item}
                    </button>
                  )
                })}
      ...
      {selectedIngredients.length > 0 && (
        <div className="px-4">
          {perfectMatches.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-800 mb-2">✅ Alles da! ({perfectMatches.length})</h3>
              <div className="grid grid-cols-2 gap-3">
                {perfectMatches.map(m => (
                  <div key={m.recipe.id} className="relative">
                    <RecipeCard recipe={m.recipe} />
                    <div className="absolute top-2 right-2 bg-green-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">100%</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {almostMatches.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-800 mb-2">🛒 Fast komplett ({almostMatches.length})</h3>
              <div className="grid grid-cols-2 gap-3">
                {almostMatches.map(m => (
                  <div key={m.recipe.id} className="relative">
                    <RecipeCard recipe={m.recipe} />
                    <div className="absolute top-2 right-2 bg-orange-400 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{m.missing === 1 ? '1 fehlt' : m.missing + ' fehlen'}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {otherMatches.length > 0 && (
            <div className="mb-6">
              <h3 className="text-sm font-semibold text-gray-800 mb-2">💡 Weitere Ideen ({otherMatches.length})</h3>
              <div className="grid grid-cols-2 gap-3">
                {otherMatches.map(m => (
                  <div key={m.recipe.id} className="relative">
                    <RecipeCard recipe={m.recipe} />
                    <div className="absolute top-2 right-2 bg-gray-400 text-white text-[10px] font-bold px-2 py-0.5 rounded-full">{m.percentage}%</div>
                  </div>
                ))}
              </div>
            </div>
