...
import { getActivityById, getTranslatedActivity, getTranslatedCategoryInfo } from '../data/activities'
...
export default function ActivityDetailPage() {
  ...
  const rawActivity = getActivityById(id || '')
  const activity = rawActivity ? getTranslatedActivity(rawActivity, language) : null
  ...
  const cat = translatedCategoryInfo[activity.category]
    ...
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="pb-8">
      ...
      <div className="mx-4 mt-4 bg-tuki-mint-bg border border-tuki-mint rounded-2xl p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-xs text-tuki-rot-dark">{t.activityDetail.tukiTip}</h3>
      ...
      <div className="px-4 mt-6">
        <h2 className="font-semibold text-base text-gray-800 mb-3">{t.activityDetail.learningGoals}</h2>
      ...
      {/* Materials */}
      <div className="px-4 mt-6">
        <h2 className="font-semibold text-base text-gray-800 mb-3">{t.activityDetail.materials}</h2>
      ...
      <div className="px-4 mt-6">
        ...
        >
          ð {t.stepMode.startActivity}
        </motion.button>
      </div>

      {/* Steps */}
      <div className="px-4 mt-6">
        <h2 className="font-semibold text-base text-gray-800 mb-3">{t.activityDetail.steps}</h2>
      ...
      <AnimatePresence>
        {showActivityMode && (
          <StepByStepMode
            ...
            onClose={() => setShowActivityMode(false)}
          />
        )}
      </AnimatePresence>
    </motion.div>
